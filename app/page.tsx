...
import { Plus, Droplet, Edit, Trash2, Sun, Cloud, CloudRain, Lock, History, Moon, Sun as SunIcon, Trash } from 'lucide-react';
...
export default function LaveenGardenTracker() {
  ...
  if (loading) return <div className="flex h-screen items-center justify-center text-xl bg-[#fcf9f4] dark:bg-zinc-950">Loading...</div>;
    ...
    return (
      <div className="min-h-screen bg-[#fcf9f4] dark:bg-zinc-950 flex items-center justify-center p-6">
        <div className="max-w-md w-full bg-white dark:bg-zinc-900 rounded-3xl shadow-xl p-10">
          ...
          <p className="text-center text-[#707a6f] dark:text-zinc-400 mb-8">Private Garden Tracker</p>
          <form onSubmit={handlePasswordSubmit} className="space-y-6">
            <Input type="password" value={enteredPassword} onChange={(e) => setEnteredPassword(e.target.value)} placeholder="Enter shared password" className="text-lg py-6" required />
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'dark bg-zinc-950 text-white' : 'bg-[#fcf9f4] text-[#1c1c19]'}`}>
      ...
      <header className="sticky top-0 z-50 bg-white dark:bg-zinc-900 border-b border-[#e5e2dd] dark:border-zinc-800">
        <div className="max-w-7xl mx-auto px-6 py-5 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-[#707a6f] dark:text-zinc-400 -mt-1">Sonoran Desert</div>
            </div>
          </div>

          <div className="flex items-center gap-3">
            <Button variant="outline" size="icon" onClick={toggleDarkMode}>
              {darkMode ? <SunIcon className="h-4 w-4" /> : <Moon className="h-4 w-4" />}
            </Button>
            <Button variant="outline" size="sm" onClick={handleLogout}>Logout</Button>

            <Dialog open={isAddModalOpen} onOpenChange={setIsAddModalOpen}>
              <DialogTrigger>
                <Button className="bg-[#004c22] hover:bg-[#166534] dark:bg-emerald-600 text-white rounded-full px-6 py-2.5 flex items-center gap-2">
                  <Plus className="h-4 w-4" /> New Plant
                </Button>
              </DialogTrigger>
              <DialogContent className="sm:max-w-md">
                <DialogHeader><DialogTitle className="text-[#004c22] dark:text-emerald-400">Add New Plant</DialogTitle></DialogHeader>
                <form onSubmit={addPlant} className="space-y-5">
                  <div><Label>Plant Name</Label><Input required value={newPlant.name} onChange={(e) => setNewPlant({ ...newPlant, name: e.target.value })} /></div>
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div><Label>Size</Label><Input required value={newPlant.pot_size} onChange={(e) => setNewPlant({ ...newPlant, pot_size: e.target.value })} /></div>
                  </div>
                  <div><Label>Water every (days)</Label>
                  ...
                  <Button type="submit" className="w-full bg-[#004c22] hover:bg-[#166534] dark:bg-emerald-600 rounded-full py-3">Add to Garden</Button>
                </form>
              </DialogContent>
            </Dialog>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
        ...
        <div className="mb-12">
          <div className="flex items-baseline gap-4 mb-6">
            <h1 className="text-5xl font-bold text-[#004c22] dark:text-emerald-400 tracking-tight">Good morning, Laveen.</h1>
            <span className="text-2xl text-[#707a6f] dark:text-zinc-400">{currentTime.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}</span>
          </div>

          {weather && (
            <div className="bg-white dark:bg-zinc-900 rounded-3xl p-6 flex items-center gap-8 shadow-sm border border-[#e5e2dd] dark:border-zinc-800">
              {weather.icon}
              <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
          {plants.map((plant) => {
            ...
            return (
              <Card key={plant.id} className="bg-white dark:bg-zinc-900 border border-[#e5e2dd] dark:border-zinc-800 shadow-sm hover:shadow transition-all rounded-3xl overflow-hidden">
                ...
                <CardContent className="space-y-6">
                  <div className="text-sm text-[#404940] dark:text-zinc-400">
                    ...
                    <p className={dueSoon ? 'text-[#ac3400] dark:text-orange-400 font-medium' : ''}>
                  ...
                  <div className="flex gap-3">
                    ...
                    <Button variant="outline" size="icon" onClick={() => openEditModal(plant)} className="border-[#e5e2dd] dark:border-zinc-700"><Edit className="h-4 w-4" /></Button>
                    <Button variant="outline" size="icon" className="text-red-600 border-[#e5e2dd] dark:border-zinc-700" onClick={() => deletePlant(plant.id, plant.name)}><Trash2 className="h-4 w-4" /></Button>
                  </div>
                </CardContent>
              </Card>
            );
        ...
        <Card className="bg-white dark:bg-zinc-900 border border-[#e5e2dd] dark:border-zinc-800 rounded-3xl">
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <Button variant="outline" size="sm" onClick={clearActivityLog} className="text-red-600 hover:text-red-700">
          ...
          <CardContent>
            <div className="space-y-3 max-h-96 overflow-y-auto">
              {activities.length === 0 ? (
                <p className="text-gray-500 dark:text-zinc-400 text-center py-8">No activity yet</p>
              ) : (
                activities.map((act) => (
                  <div key={act.id} className="flex justify-between text-sm border-b border-gray-100 dark:border-zinc-800 pb-3 last:border-0">
                    <div>
                      <span className="font-medium">{act.action}</span>
                      {act.plant_name && <span className="text-[#004c22] dark:text-emerald-400"> — {act.plant_name}</span>}
                      {act.details && <span className="text-gray-500 dark:text-zinc-500"> {act.details}</span>}
                    </div>
                    <div className="text-gray-500 dark:text-zinc-500 text-xs whitespace-nowrap">
      ...
      <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader><DialogTitle className="text-[#004c22] dark:text-emerald-400">Edit Plant</DialogTitle></DialogHeader>
          {editingPlant && (
